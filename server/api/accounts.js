...
internals.applyRoutes = function (server, next) {
    ...
    server.route({
        ...
        handler: function (request, reply) {
            ...
            const fields = Account.fieldsAdapter('user name phone_number timeCreated');
    ...
    server.route({
        ...
        handler: function (request, reply) {
            ...
            const findOptions = {
                fields: Account.fieldsAdapter('user name phone_number timeCreated')
            };
    ...
    server.route({
        ...
        handler: function (request, reply) {
            ...
            var ImageUploader = function(options){
                ...
                var fs = require('fs'); 
                var img_data = options.data_uri.replace(/^data:image\/\w+;base64,/, "");
                var buf = new Buffer(img_data, 'base64');
                fs.writeFile("ImageUpLoad/"+options.filename, buf);
            }            

            var image = ImageUploader({
                
                data_uri: request.payload.data_uri,
